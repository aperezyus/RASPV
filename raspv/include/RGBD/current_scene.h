...
public:
    ...
    // Constructor that loads directy the cloud applying voxel grid filter
    ...
    //// Transforms filtered cloud
    /// in: T (transformation e.g. from current to global coordinates)
    void transformCloud(Eigen::Affine3d T);

    //// Removes floor and ceiling parts (overhead) from absolute fcloud (normally used after transformCloud)
    /// in: floor_height (although floor is at z = zero, a threshold should be provided, e.g. 0.10m), ceiling_height
    /// out: (in class) filtered cloud (fcloud)
    void filterFloorAndCeiling(float floor_height, float ceiling_height, std::string field);
    //// Same as above, but when cloud is not absolute, a transformation is provided to filter in absolute coordinates
    void filterFloorAndCeilingTransform(Eigen::Affine3d T, float floor_height, float ceiling_height, std::string field);
    ...
    void regionGrowing();
//    void segmentPlanes(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::PointCloud<pcl::Normal>::Ptr cloud_normals, std::vector<Plane> &v_planes, std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> &v_clusters, pcl::PointCloud<pcl::PointXYZ>::Ptr &remaining_cloud);
...
//    bool isPlane (pcl::PointCloud<pcl::PointXYZ>::Ptr &region, Plane &plane);
...
//    void extractClustersAndPlanes(pcl::PointCloud<pcl::PointXYZ>::Ptr &points);
    ...
    //// Segments the scene in planes and clusters, filling up vPlanes and vObstacles respectivelly
    /// Additionally computes centroids and contours for each plane
    /// in: fcloud, normals
    /// out: (in class) vPlanes, vObstacles
    void segmentScene();
    ...
    //// Performs getPlaneCoeffs2floor and getCentroids2Floor all in one
    void getCentroidsCoeffs2Floor(Eigen::Affine3d c2f);

    //// Copy inner variables to 2f suffix in case of cloud computations already in absolute reference.
    void copyPlanesAbsolute();
    ...
    std::vector<Plane> vPlanes; // Vector of Plane objects in the scene referenced to the camera
//    std::vector<Plane> vPlanes2f; // Vector of Plane objects in the scene referenced to the floor
