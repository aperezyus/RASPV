...
void GlobalScene::findFloor(CurrentScene &scene) {
    if (!initial_floor_)
        this->findFloorCloud(scene.fcloud);
    else
        this->findFloorFast(scene.vPlanes);

}



void GlobalScene::findFloorCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud) {
    ...
    if (initial_floor_)
        this->computeCamera2FloorMatrix(floor_normal_);
}
	...
	*cloud_filtered=*cloud;    
    ...
    if (cloud_filtered->points.size()>k_min_plane_size){ // Three points at least to find a plane
        ...
        if (inliers->indices.size() > k_min_plane_size ||
            ...
            const float k_angle_threshold = 60; // Valid threshold around estimated floor vector
            const float k_min_D = 0.9f; // Minimum D value (i.e. minimum distance of the floor to the camera allowed)
...
void GlobalScene::computeCamera2AbsoluteMatrix() {

  c2a.matrix().row(0) = c2f.matrix().row(2);
  c2a.matrix().row(1) = c2f.matrix().row(0);
  c2a.matrix().row(2) = c2f.matrix().row(1);
...
void GlobalScene::computeIncrementalCamera2FloorMatrix (Eigen::Vector4f floor_normal) {
